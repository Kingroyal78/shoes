...
| `scripts/v2board_e2e_matrix.sh` | 82-case default inbound matrix for VMess, VLESS, Shadowsocks, Trojan, AnyTLS, TUIC, Hysteria2, V2Node variants, transports, TLS/Reality/Vision, and PROXY protocol, including a VLESS UUID v1 interoperability case. The exact default list is authoritative in `E2E_MATRIX_CASES` inside the script. The historical raw `/config` `shadowsocks_obfs_http` case is quarantined because that endpoint intentionally returns null legacy obfs fields; it is not a plugin acceptance case. Plugin acceptance uses the versioned `plugin-config`/`status` contract and `scripts/e2e_ss_plugins_mihomo.sh`. WebSocket cases prove binary proxy payload interoperability but are not an external RFC 6455 conformance suite. The TUIC zero-RTT configuration case does not establish a session ticket and send a pre-authentication QUIC-stream Packet command. Trojan cases do not exercise the optional probe fallback, and Hysteria2 cases do not exercise the optional static ordinary-H3 masquerade. |
| `scripts/v2board_e2e_dedicated_upstream.sh` | Per-user dedicated egress through a bought upstream proxy. Stands up a real SOCKS5 proxy on a second loopback address, seeds an `upstream_proxy` pool through the panel's own services (so the credential is encrypted with the deployment's key rather than a fake), asserts the panel publishes `mode: proxy` with the dialing credentials on `/user`, then drives real traffic through an external Mihomo client into the node and checks the probe saw the upstream's address. Also asserts UDP is refused rather than falling back to the node's own address, and that releasing the assignment returns the user to the default egress. Reserved node/user/pool IDs are 9702/19702/9702 and ports 18811-18814. |
| `scripts/v2board_e2e_dedicated_ip.sh` | Per-user dedicated egress IP, source-binding form. Seeds a `v2_dedicated_ip_pool`/`v2_dedicated_ip`/`v2_dedicated_ip_assignment` set bound to a VMess node, asserts the panel publishes `dedicated_ip` on `/user`, then drives real traffic through an external Mihomo client to an HTTP probe that echoes the source address it observed -- so the assertion is on what the peer saw, not on anything shoes reports about itself. Releasing the assignment must return the user to the default source on the next pull. Uses loopback addresses (`127.0.0.9` by default), which need no interface setup on Linux. Reserved node/user/pool IDs are 9701/19701/9701 and ports 18801-18803. |
...
| `E2E_MATRIX_CASES` | 82-case comma-separated list defined in `v2board_e2e_matrix.sh` | Override the default inbound protocol matrix. Keep the script as the authoritative list to avoid documentation drift. |
