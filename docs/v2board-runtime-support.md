...
| Restls script beyond the v1-safe range | Rejected | The panel accepts scripts up to record target 32767 / endpoint 32768 / 254 responses and gates them on `shadowsocks-plugin-restls-v2`. This backend implements the One/Shoes v1-safe range only (last record target 16364, 127 responses) and advertises `shadowsocks-plugin-restls-v1`; a wider script is rejected by name and the last-known-good runtime is kept. |
