...
E2E_HTTP_PORT="${E2E_HTTP_PORT:-18113}"
...
E2E_WAIT_TIMEOUT_SECS="${E2E_WAIT_TIMEOUT_SECS:-30}"
E2E_KEEP_FIXTURES="${E2E_KEEP_FIXTURES:-1}"
E2E_CURL_CONNECT_TIMEOUT_SECS="${E2E_CURL_CONNECT_TIMEOUT_SECS:-3}"
E2E_CURL_MAX_TIME_SECS="${E2E_CURL_MAX_TIME_SECS:-10}"

GROUP_ID=9913
NODE_ID=9913
USER_ID=19913
USER_UUID=99999999-9999-4999-8999-999999999913
REMOTE_NODE_ID=9914
...
Runs a real V2Board global device-limit admission check:
  - seeds a VMess user with device_limit=1
  - preloads V2Board ALIVE_IP_USER_<uid> cache with alive_ip=1
  - starts shoes and verifies a new connection is rejected after /alivelist sync
EOF
}
...
cache_put_remote_alive() {
  docker exec "${V2BOARD_WWW_CONTAINER}" \
    php artisan tinker --execute="Cache::put('ALIVE_IP_USER_${USER_ID}', ['vmess${REMOTE_NODE_ID}' => ['aliveips' => ['198.51.100.7_${REMOTE_NODE_ID}'], 'lastupdateAt' => time()], 'alive_ip' => 1], 120); Cache::forget('ALIVE_LIST');" \
    >/dev/null
}

start_http_target() {
  e2e_section "start global-device-limit http target"
  e2e_assert_port_free "${E2E_HTTP_PORT}" "global-device-limit http target"
  mkdir -p "${TMP_DIR}/http-root"
  printf 'device limit should block this request\n' >"${TMP_DIR}/http-root/payload.txt"
  ...
  HTTP_PID=$!
  wait_for_listen_port "${E2E_HTTP_PORT}" "global-device-limit http target" 10
}
...
VALUES
(${GROUP_ID}, 'shoes-global-device-limit', ${now}, ${now})
...
VALUES
(${NODE_ID}, '["${GROUP_ID}"]', NULL, 'shoes-global-device-limit', 'US', 'Local', NULL, NULL, '${E2E_BIND_HOST}', '${E2E_NODE_PORT}', ${E2E_NODE_PORT}, 0, NULL, '1', 'tcp', NULL, '{}', '{}', '{}', '{}', 1, ${NODE_ID}, ${now}, ${now})
ON DUPLICATE KEY UPDATE
...
VALUES
(${USER_ID}, NULL, NULL, 'shoes-global-device-limit@example.local', NULL, 'e2e-password', NULL, NULL, 0, NULL, 0, NULL, 0, 0, 0, 0, 1073741824, 1, 0, 0, NULL, 0, NULL, '${USER_UUID}', ${GROUP_ID}, NULL, NULL, 0, 1, 1, MD5('shoes-global-device-limit@example.local'), ${expires_at}, 'shoes global device limit e2e', ${now}, ${now})
ON DUPLICATE KEY UPDATE
  banned=0,
...
write_configs() {
  cat >"${TMP_DIR}/global-device-limit.shoes.yml" <<YAML
v2board:
  api_host: "${V2BOARD_PANEL_URL}"
  api_key: "${SERVER_TOKEN}"
  nodes:
    - tag: "global_device_limit"
      node_id: ${NODE_ID}
      node_type: "vmess"
...
runtime:
  data_dir: "${TMP_DIR}/global-device-limit-shoes-data"
  pull_interval_secs: ${E2E_PULL_INTERVAL_SECS}
  push_interval_secs: ${E2E_PUSH_INTERVAL_SECS}
...
YAML
  ...
  cat >"${TMP_DIR}/global-device-limit.singlink.json" <<JSON
{
...
wait_for_alivelist_count() {
  ...
  while true; do
    cache_forget "ALIVE_LIST"
    json="$(curl -fsS --max-time 5 "${V2BOARD_PANEL_URL%/}/api/v1/server/UniProxy/alivelist?token=${SERVER_TOKEN}&node_id=${NODE_ID}&node_type=vmess")"
    if ALIVE_LIST_JSON="${json}" python3 - "${USER_ID}" <<'PY'
import json
import os
import sys
...
if not isinstance(alive, dict) or int(alive.get(uid, -1)) != 1:
    raise SystemExit(1)
PY
    then
      e2e_log "alivelist seeded: ${json}"
      return
    fi
    now="$(date +%s)"
    if ((now - start >= E2E_WAIT_TIMEOUT_SECS)); then
      e2e_die "alivelist did not expose seeded alive count; last=${json}"
    fi
    sleep 1
  done
}
...
start_services() {
  "${SHOES_BIN}" sync-once -c "${TMP_DIR}/global-device-limit.shoes.yml" >"${TMP_DIR}/global-device-limit.sync.log" 2>&1
  e2e_assert_port_free "${E2E_NODE_PORT}" "global-device-limit shoes"
  "${SHOES_BIN}" run -c "${TMP_DIR}/global-device-limit.shoes.yml" >"${TMP_DIR}/global-device-limit.shoes.log" 2>&1 &
  SHOES_PID=$!
  wait_for_listen_port "${E2E_NODE_PORT}" "global-device-limit shoes" 15

  "${SINGLINK_BIN}" -c "${TMP_DIR}/global-device-limit.singlink.json" check >"${TMP_DIR}/global-device-limit.singlink-check.log" 2>&1
  e2e_assert_port_free "${E2E_PROXY_PORT}" "global-device-limit singlink"
  "${SINGLINK_BIN}" -c "${TMP_DIR}/global-device-limit.singlink.json" run >"${TMP_DIR}/global-device-limit.singlink.log" 2>&1 &
  SINGLINK_PID=$!
  wait_for_listen_port "${E2E_PROXY_PORT}" "global-device-limit singlink" 15
}
...
assert_proxy_blocked_by_global_device_limit() {
  local status
  ...
  if curl -fsS \
    ...
    "http://${E2E_BIND_HOST}:${E2E_HTTP_PORT}/payload.txt" \
    -o "${TMP_DIR}/blocked-download.txt" \
    >"${TMP_DIR}/blocked.curl.out" \
    2>"${TMP_DIR}/blocked.curl.err"; then
    e2e_die "proxy request unexpectedly succeeded despite global device_limit"
  else
    status=$?
  fi

  if ! grep -Fq "device limit exceeded for user ${USER_ID}" "${TMP_DIR}/global-device-limit.shoes.log"; then
    e2e_die "shoes log did not contain global device_limit rejection"
  fi
  e2e_log "proxy request rejected by global device_limit as expected (curl status ${status})"
}

cleanup_fixtures() {
  if e2e_env_bool E2E_KEEP_FIXTURES 1; then
    e2e_log "keeping global-device-limit E2E fixtures"
    return
  fi
...
DELETE FROM v2_stat_user WHERE user_id=${USER_ID};
DELETE FROM v2_stat_server WHERE server_id=${NODE_ID} AND server_type='vmess';
DELETE FROM v2_user WHERE email='shoes-global-device-limit@example.local';
DELETE FROM v2_server_vmess WHERE name='shoes-global-device-limit';
DELETE FROM v2_server_group WHERE name='shoes-global-device-limit';
SQL
  e2e_redis_hdel_user_traffic "${V2BOARD_REDIS_CONTAINER}" "${USER_ID}"
  cache_forget "ALIVE_IP_USER_${USER_ID}"
  cache_forget "ALIVE_LIST"
...
main() {
  ...
  TMP_DIR="$(mktemp -d /tmp/shoes-v2board-global-device-limit-e2e.XXXXXX)"
