...
E2E_HTTP_PORT="${E2E_HTTP_PORT:-18096}"
E2E_NODE_PORT="${E2E_NODE_PORT:-18172}"
E2E_BAD_NODE_PORT="${E2E_BAD_NODE_PORT:-18173}"
E2E_PROXY_PORT="${E2E_PROXY_PORT:-18272}"
...
GROUP_ID=9802
USER_ID=19802
USER_UUID=88888002-9802-4888-8888-888888888802
...
Runs a real V2Board reload rollback E2E check:
  - starts a VMess/TCP node from panel config
  - proves the old listener proxies traffic
  - updates the panel node to a port already occupied by another process
  - verifies shoes logs rollback and the old listener still proxies traffic
EOF
}
...
start_http_target() {
  ...
  e2e_assert_port_free "${E2E_HTTP_PORT}" "reload http target"
  ...
  HTTP_PID=$!
  wait_for_listen_port "${E2E_HTTP_PORT}" "reload http target" 10
}
...
VALUES
(${GROUP_ID}, 'shoes-reload-user', ${now}, ${now})
...
VALUES
(${NODE_ID}, '["${GROUP_ID}"]', NULL, 'shoes-reload-user', 'US', 'Local', NULL, NULL, '${E2E_BIND_HOST}', '${E2E_NODE_PORT}', ${E2E_NODE_PORT}, 0, NULL, '1', 'tcp', NULL, '{}', '{}', '{}', '{}', 1, ${NODE_ID}, ${now}, ${now})
ON DUPLICATE KEY UPDATE
...
VALUES
(${USER_ID}, NULL, NULL, 'shoes-reload-user@example.local', NULL, 'e2e-password', NULL, NULL, 0, NULL, 0, NULL, 0, 0, 0, 0, 1073741824, NULL, 0, 0, NULL, 0, NULL, '${USER_UUID}', ${GROUP_ID}, NULL, NULL, 0, 1, 1, MD5('shoes-reload-user@example.local'), ${expires_at}, 'shoes reload rollback e2e', ${now}, ${now})
ON DUPLICATE KEY UPDATE
  banned=0,
...
write_configs() {
  cat >"${TMP_DIR}/reload.shoes.yml" <<YAML
v2board:
...
YAML
  ...
  cat >"${TMP_DIR}/reload.singlink.json" <<JSON
{
...
start_services() {
  e2e_assert_port_free "${E2E_NODE_PORT}" "reload shoes"
  e2e_assert_port_free "${E2E_PROXY_PORT}" "reload singlink"

  "${SHOES_BIN}" run -c "${TMP_DIR}/reload.shoes.yml" >"${TMP_DIR}/reload.shoes.log" 2>&1 &
  SHOES_PID=$!
  wait_for_listen_port "${E2E_NODE_PORT}" "reload shoes" 15

  "${SINGLINK_BIN}" -c "${TMP_DIR}/reload.singlink.json" check >"${TMP_DIR}/reload.singlink-check.log" 2>&1
  "${SINGLINK_BIN}" -c "${TMP_DIR}/reload.singlink.json" run >"${TMP_DIR}/reload.singlink.log" 2>&1 &
  SINGLINK_PID=$!
  wait_for_listen_port "${E2E_PROXY_PORT}" "reload singlink" 15
}

start_port_blocker() {
  e2e_assert_port_free "${E2E_BAD_NODE_PORT}" "reload bad port"
  cat >"${TMP_DIR}/port_blocker.py" <<'PY'
import socket
import sys
import time
...
while True:
    time.sleep(3600)
PY
  python3 "${TMP_DIR}/port_blocker.py" "${E2E_BIND_HOST}" "${E2E_BAD_NODE_PORT}" >"${TMP_DIR}/port_blocker.log" 2>&1 &
  BLOCKER_PID=$!
  wait_for_listen_port "${E2E_BAD_NODE_PORT}" "reload bad port blocker" 10
}
...
wait_for_rollback_log() {
  ...
  start="$(date +%s)"
  while true; do
    if grep -q "restored previous V2Board node" "${TMP_DIR}/reload.shoes.log"; then
      e2e_log "reload rollback log observed"
      return
    fi
    if [[ -n "${SHOES_PID}" ]] && ! kill -0 "${SHOES_PID}" 2>/dev/null; then
      e2e_die "shoes exited while waiting for rollback"
    fi
    now="$(date +%s)"
    if ((now - start >= E2E_WAIT_TIMEOUT_SECS)); then
      e2e_die "reload rollback log was not observed"
    fi
    sleep 1
  done
}
...
stop_services() {
  for pid_var in SINGLINK_PID SHOES_PID BLOCKER_PID HTTP_PID; do
    local pid="${!pid_var:-}"
    if [[ -n "${pid}" ]] && kill -0 "${pid}" 2>/dev/null; then
...
cleanup_fixtures() {
  if e2e_env_bool E2E_KEEP_FIXTURES 1; then
    e2e_log "keeping reload E2E fixtures"
    return
  fi
...
DELETE FROM v2_user WHERE email='shoes-reload-user@example.local';
DELETE FROM v2_server_vmess WHERE name='shoes-reload-user';
DELETE FROM v2_server_group WHERE name='shoes-reload-user';
SQL
...
main() {
  ...
  TMP_DIR="$(mktemp -d /tmp/shoes-v2board-reload-e2e.XXXXXX)"
  ...
  update_node_to_blocked_port
  wait_for_rollback_log
  wait_for_listen_port "${E2E_NODE_PORT}" "reload restored shoes" 10
