...
Runs real V2Board TUIC policy E2E checks:
  - wrong TUIC password fails and records no traffic.
  - speed_limit=1 Mbps throttles a 1 MiB transfer.
  - V2Node TUIC speed_limit=1 Mbps uses the same user policy.
  - device_limit=1 rejects a second concurrent QUIC source IP.
  - V2Node TUIC device_limit=1 rejects a second concurrent QUIC source IP.
EOF
}
...
start_policy_http() {
  e2e_section "start tuic policy http target"
  e2e_assert_port_free "${E2E_HTTP_PORT}" "tuic policy http target"
...
PY
  ...
  HTTP_PID=$!
  wait_for_listen_port "${E2E_HTTP_PORT}" "tuic policy http target" 10
}
...
VALUES
(${node_id}, '["1"]', NULL, 'shoes-tuic-policy-${case_name}', 'US', 'Local', NULL, NULL, '${E2E_BIND_HOST}', '${port}', ${port}, NULL, '1', 1, ${node_id}, '${E2E_TLS_SERVER_NAME}', 0, 0, 'native', 0, 'cubic', ${now}, ${now})
...
VALUES
(${user_id}, NULL, NULL, 'shoes-tuic-policy-${case_name}@example.local', NULL, 'e2e-password', NULL, NULL, 0, NULL, 0, NULL, 0, 0, 0, 0, 1073741824, ${device_limit}, 0, 0, NULL, 0, NULL, '${uuid}', 1, NULL, ${speed_limit}, 0, 1, 1, MD5('shoes-tuic-policy-${case_name}@example.local'), ${expires_at}, 'shoes tuic policy ${case_name} e2e user', ${now}, ${now})
ON DUPLICATE KEY UPDATE
  banned=0,
...
VALUES
(${node_id}, '["1"]', NULL, 'shoes-tuic-policy-${case_name}', 'US', 'Local', NULL, NULL, '${E2E_BIND_HOST}', '${E2E_BIND_HOST}', '${port}', ${port}, NULL, '1', 1, ${node_id}, 'tuic', 1, '{}', NULL, 'tcp', '{}', 'none', '{}', 0, 'native', 0, 'cubic', NULL, 0, 0, NULL, NULL, NULL, ${now}, ${now})
ON DUPLICATE KEY UPDATE
  group_id=VALUES(group_id),
  name=VALUES(name),
  host=VALUES(host),
...
(id, invite_user_id, telegram_id, email, language, password, password_algo, password_salt, balance, discount, commission_type, commission_rate, commission_balance, t, u, d, transfer_enable, device_limit, banned, is_admin, last_login_at, is_staff, last_login_ip, uuid, group_id, plan_id, speed_limit, auto_renewal, remind_expire, remind_traffic, token, expired_at, remarks, created_at, updated_at)
VALUES
(${user_id}, NULL, NULL, 'shoes-tuic-policy-${case_name}@example.local', NULL, 'e2e-password', NULL, NULL, 0, NULL, 0, NULL, 0, 0, 0, 0, 1073741824, ${device_limit}, 0, 0, NULL, 0, NULL, '${uuid}', 1, NULL, ${speed_limit}, 0, 1, 1, MD5('shoes-tuic-policy-${case_name}@example.local'), ${expires_at}, 'shoes v2node tuic policy ${case_name} e2e user', ${now}, ${now})
ON DUPLICATE KEY UPDATE
  banned=0,
  transfer_enable=VALUES(transfer_enable),
  u=0,
...
wait_for_policy_download() {
  ...
  while true; do
    ...
    if ((user_d >= expected_min && stat_user_d >= expected_min && stat_server_d >= expected_min)); then
      e2e_log "tuic policy server_type=${server_type} user=${user_u}/${user_d} stat_user=${stat_user_u}/${stat_user_d} stat_server=${stat_server_u}/${stat_server_d}"
      return
    fi
    now="$(date +%s)"
    if ((now - start >= E2E_WAIT_TIMEOUT_SECS)); then
      e2e_die "TUIC user ${user_id} download did not reach ${expected_min}; server_type=${server_type} user=${user_u}/${user_d} stat_user=${stat_user_u}/${stat_user_d} stat_server=${stat_server_u}/${stat_server_d}"
    fi
    sleep 1
  done
}
...
assert_no_traffic() {
  ...
  stat_server="$(mysql_query "SELECT COALESCE(SUM(u+d),0) FROM v2_stat_server WHERE server_id=${node_id} AND server_type='tuic';")"
...
run_auth_failure_case() {
  ...
  local node_id=9551
  local user_id=19551
  local uuid=55555555-5555-4555-8555-555555555551
  local node_port=18146
  local proxy_port=18246

  e2e_section "tuic policy auth_failure"
  seed_tuic_fixture "${case_name}" "${node_id}" "${user_id}" "${uuid}" "${node_port}" NULL NULL
  ...
  if curl -fsS \
    ...
    e2e_die "TUIC auth_failure unexpectedly downloaded payload"
  fi
  assert_no_traffic "${user_id}" "${node_id}"
  e2e_log "tuic auth_failure rejected wrong password as expected"
...
run_speed_limit_case() {
  ...
  local node_id=9552
  local user_id=19552
  local uuid=55555555-5555-4555-8555-555555555552
  local node_port=18147
  local proxy_port=18247
  local start
  local end
  local elapsed_ms

  e2e_section "tuic policy speed_limit"
  seed_tuic_fixture "${case_name}" "${node_id}" "${user_id}" "${uuid}" "${node_port}" 1 NULL
  write_shoes_config "${case_name}" "${node_id}" "${node_port}"
  ...
  [[ "$(wc -c <"${TMP_DIR}/${case_name}.bin")" -eq 1048576 ]] \
    || e2e_die "tuic speed_limit download size mismatch"
  ((elapsed_ms >= 5000)) \
    || e2e_die "tuic speed_limit did not throttle enough: elapsed ${elapsed_ms}ms, expected >= 5000ms"
  wait_for_policy_download "${user_id}" "${node_id}" 1048576
  e2e_log "tuic speed_limit elapsed=${elapsed_ms}ms"
...
run_v2node_speed_limit_case() {
  ...
  local node_id=9651
  local user_id=19651
  local uuid=66666666-6666-4666-8666-666666666651
  local node_port=18551
  local proxy_port=18651
  local start
  local end
  local elapsed_ms

  e2e_section "tuic policy v2node_speed_limit"
  seed_v2node_tuic_fixture "${case_name}" "${node_id}" "${user_id}" "${uuid}" "${node_port}" 1 NULL
  ...
  [[ "$(wc -c <"${TMP_DIR}/${case_name}.bin")" -eq 1048576 ]] \
    || e2e_die "tuic v2node_speed_limit download size mismatch"
  ((elapsed_ms >= 5000)) \
    || e2e_die "tuic v2node_speed_limit did not throttle enough: elapsed ${elapsed_ms}ms, expected >= 5000ms"
  wait_for_policy_download "${user_id}" "${node_id}" 1048576 v2node
  e2e_log "tuic v2node_speed_limit elapsed=${elapsed_ms}ms"
...
run_device_limit_case() {
  ...
  local node_id=9553
  local user_id=19553
  local uuid=55555555-5555-4555-8555-555555555553
  local node_port=18148
  local proxy_a=18248
  local proxy_b=18249
  local slow_pid

  e2e_section "tuic policy device_limit"
  seed_tuic_fixture "${case_name}" "${node_id}" "${user_id}" "${uuid}" "${node_port}" NULL 1
  write_shoes_config "${case_name}" "${node_id}" "${node_port}"
  ...
  sleep 1
  kill -0 "${slow_pid}" 2>/dev/null || e2e_die "first TUIC slow device-limit connection exited too early"
  ...
  if curl -fsS \
    ...
    e2e_die "tuic device_limit allowed a second concurrent source IP"
  fi
  ...
  grep -q "device limit exceeded for user ${user_id}" "${TMP_DIR}/${case_name}.shoes.log" \
    || e2e_die "tuic device_limit rejection was not observed in shoes log"
  e2e_log "tuic device_limit rejected second source IP as expected"
...
run_v2node_device_limit_case() {
  ...
  local node_id=9652
  local user_id=19652
  local uuid=66666666-6666-4666-8666-666666666652
  local node_port=18552
  local proxy_a=18652
  local proxy_b=18653
  local slow_pid

  e2e_section "tuic policy v2node_device_limit"
  seed_v2node_tuic_fixture "${case_name}" "${node_id}" "${user_id}" "${uuid}" "${node_port}" NULL 1
  write_shoes_config "${case_name}" "${node_id}" "${node_port}" v2node
  write_singlink_config "${case_name}" "${uuid}" "${uuid}" "${node_port}" "${proxy_a}" "127.0.0.1"
  write_singlink_config "${case_name}" "${uuid}" "${uuid}" "${node_port}" "${proxy_b}" "127.0.0.2"
  start_singlink "${case_name}" "${proxy_a}"
  ...
  sleep 1
  kill -0 "${slow_pid}" 2>/dev/null || e2e_die "first V2Node TUIC slow device-limit connection exited too early"
  ...
  if curl -fsS \
    ...
    kill "${slow_pid}" 2>/dev/null || true
    wait "${slow_pid}" 2>/dev/null || true
    e2e_die "tuic v2node_device_limit allowed a second concurrent source IP"
  fi
  ...
  grep -q "device limit exceeded for user ${user_id}" "${TMP_DIR}/${case_name}.shoes.log" \
    || e2e_die "tuic v2node_device_limit rejection was not observed in shoes log"
  e2e_log "tuic v2node_device_limit rejected second source IP as expected"
...
maybe_cleanup_fixtures() {
  if e2e_env_bool E2E_KEEP_FIXTURES 1; then
    e2e_log "keeping TUIC policy E2E fixtures"
    return
  fi

  mysql_exec <<SQL
DELETE su FROM v2_stat_user su JOIN v2_user u ON u.id=su.user_id WHERE u.email LIKE 'shoes-tuic-policy-%@example.local';
DELETE ss FROM v2_stat_server ss JOIN v2_server_tuic n ON ss.server_type='tuic' AND ss.server_id=n.id WHERE n.name LIKE 'shoes-tuic-policy-%';
DELETE ss FROM v2_stat_server ss JOIN v2_server_v2node n ON ss.server_type='v2node' AND ss.server_id=n.id WHERE n.name LIKE 'shoes-tuic-policy-%';
DELETE FROM v2_user WHERE email LIKE 'shoes-tuic-policy-%@example.local';
DELETE FROM v2_server_tuic WHERE name LIKE 'shoes-tuic-policy-%';
DELETE FROM v2_server_v2node WHERE name LIKE 'shoes-tuic-policy-%';
SQL

  e2e_redis_hdel_user_traffic "${V2BOARD_REDIS_CONTAINER}" 19551
  e2e_redis_hdel_user_traffic "${V2BOARD_REDIS_CONTAINER}" 19552
  e2e_redis_hdel_user_traffic "${V2BOARD_REDIS_CONTAINER}" 19553
  e2e_redis_hdel_user_traffic "${V2BOARD_REDIS_CONTAINER}" 19651
  e2e_redis_hdel_user_traffic "${V2BOARD_REDIS_CONTAINER}" 19652
}

main() {
  parse_args "$@"
  e2e_validate_bool_env E2E_KEEP_FIXTURES 1
  TMP_DIR="$(mktemp -d /tmp/shoes-v2board-tuic-policy-e2e.XXXXXX)"
